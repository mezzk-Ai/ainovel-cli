...
> 修订 2026-07-16：模型档位旋钮落地为 roles 配置 `import_segment/import_analyze/import_synthesize`（§13.1）；自然语言重切分落地为 `--guide` 与工作区 `guidance.txt` 语义输入（§18.3）；语义失败统一保存原始响应到 failures/（§14.2）。JSON Schema 模式（§13.2 第 1 级）暂未实现，标记 TODO 待与全仓其它模型调用点统一改造。
...
上下文区可以重叠，但每次调用只能为 `owned_units` 返回结果，因此不存在重叠块投票或冲突合并。

分块大小由当前 architect 模型的 context window 和保留预算计算，不按固定行数或固定章节数切块。模型上下文扩大后，调用次数自然下降。
...
1. 所有 unit ID 存在且落在调用的 owned range；
2. 非空 anchor 在对应 unit 内唯一，并能映射到 UTF-8 字节边界；
3. 边界按 `(Line, Part)` 数值序严格递增，不重复（不对 ID 做字典序比较）；
4. 每个章节正文范围非空；
5. 所有非空源文本恰好属于一个章节、一个 group 标题或明确的 front/back matter；
6. 没有重叠、越界或未归属范围；
7. group 不被错误计入章节总数。
...
- 确认（TUI 预览面板按 `y`，等价于重跑 `/import --yes`；一次性放行当前切分，不写入 intent）；
- 输入自然语言说明后重新识别，例如 `/import --guide=幕间·X 也是独立章节`；
- 取消并保留工作区（Esc）。

`/import <path> --yes` 是显式无人值守授权：Runner 在覆盖校验通过后写入同样的 confirmation 工件，记录 `method=auto_authorized`，随后继续分析。`--yes` 即使存在 uncertain 边界也表示用户选择信任本次切分，但 uncertain 仍保留在工件和日志中。
...
长度截断是容量错误，不消耗“把校验错误反馈给同一模型”的三次语义重试，也不原样重试同一批次。
...
违反结构约束时把具体错误反馈给模型重新生成；超过有限结构重试后显式失败，不落盘半成品。
...
1. 明确支持 JSON Schema：发送对应 schema；（**TODO 未实现**：待与全仓其它模型调用点——arbiter 裁定、engine 语义工具等——统一改造，见 `imp/call.go` callProfile 的 TODO(json-schema)；改造前行为正确，只损失约束强度）
2. 否则明确支持 JSON Object：启用 object mode；
3. 否则使用 prompt JSON 契约，并在日志中记录 `structured_mode=prompt_only`。

无论 provider 是否约束输出，Go 都执行同一套解析和校验。不能在请求报错后静默删除参数重试；能力判断错误或 provider 拒绝必须暴露。

### 13.3 请求、语义与容量失败分离

- 请求层错误：只重试适配器明确标记 retryable 的超时、限流、网络错误，沿用现有退避语义；
- 输出层错误：把 JSON 解析或 Validate 的具体错误反馈给同一模型，最多 3 次；仍失败则终止当前动作。
- 容量错误：`StopReasonLength` 不原样重试，也不占用三次语义重试；analysis batch 在部分文本可解析时按 §9.5 保存连续合法前缀，否则记录 `prefix_salvage=unavailable` 并缩小重组批；其余语义函数直接显式失败并保留原始响应。
...
同一本书存在活动工作区时执行无参数 `/import`，直接从事实和已保存 intent 推导下一步；源文件路径和启动参数都不是恢复必需项。UI 应明确显示“发现未完成导入，正在从第 N 章分析/发布处恢复”。带新路径的 `/import <path>` 不得覆盖活动工作区。
