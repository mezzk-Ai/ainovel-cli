...
- **issues**：发现的具体问题列表
  ...
  - suggestion：修改建议
  - chapters：证据实际落在哪些章节；弧级/全局评审必须逐项精确标注
  - requires_change：这个问题是否应立即触发所列章节返工，由你结合整体阅读体验判断
...
返工章节由工具从 `issues[].requires_change=true` 的 `chapters` 自动推导，不要再提交一份重复的 `affected_chapters`。
