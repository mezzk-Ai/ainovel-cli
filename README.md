...
在 TUI 中输入 `/import <文件路径>` 可把一本已有的小说**语义编译**进项目。管线分阶段推进：源文件快照（ingest）→ LLM 识别章节边界（segment）→ 确认切分 → 逐章提取事实（analyze）→ 分层归纳全书前提 / 角色 / 世界观 / 分层大纲 / 指南针（synthesize）→ 发布正式 Foundation 并逐章落盘（publish）。章节边界由模型按语义裁定，不依赖硬编码标题规则；Go 侧只掌管坐标、覆盖校验、幂等与顺序。

```
/import ~/我的小说.txt                 # 语义导入，切分后停下等你核对（面板内按 y 确认继续）
/import ~/我的小说.txt --yes           # 授权自动接受切分并跑完全程
/import ~/我的小说.txt --story=closed  # 综合判定故事状态存疑时，明确其为完结/开放
/import ~/我的小说.txt --continue      # 导入完成后直接接力续写，不停在验收
/import                                # 无参数：恢复上次未完成的导入
/import --guide=幕间·X 也是独立章节     # 用自然语言调整切分后重新识别（置于命令最后）
```

导入是可恢复的：每个阶段的产物落在 `meta/import/` 工作区并按输入指纹绑定，中断后重跑只补做缺失的部分。默认在发布完成后设一次验收 Hold，等你确认再续写；`--continue` 跳过该 Hold。故事状态被综合判定为 `uncertain` 时，管线会停下等待 `--story=open|closed` 明确后再发布。
